...
#if defined(__ECOS__)
#define MAX_XS_BUFFER	256 //the max size for buffer io, to prevent lot of fgetc calls.
#endif
...
struct xstream
{
	...
	size_t				udsize;
#if defined(__ECOS__)
	//+++ add by siyou for buffer io, to speed up the fgetc.
	unsigned char	buffer[MAX_XS_BUFFER];
	size_t				numread;
	size_t				curr;
	//--- end add.
#endif
...
#else
	case XSTYPE_FILE:
				if ( xs->curr >= xs->numread )
				{
					xs->numread = fread(xs->buffer, 1, MAX_XS_BUFFER, xs->fd.file);
					if ( xs->numread )
						xs->curr = 0;
					else
					{
