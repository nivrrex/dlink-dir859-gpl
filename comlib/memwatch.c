...
void mwInit(void)
{
	...
	if (!mwInfoWritten) {
		...
		mwWrite("\n============="
			" MEMWATCH " VERSION
			" Copyright (C) 1992-1999 Johan Lindh "
			"=============\n");
		mwWrite("\nStarted at %s\n", ctime(&tid));
...
#ifdef __cplusplus
			"++ %d.%01d\n", __BCPLUSPLUS__ / 0x100,
			(__BCPLUSPLUS__ % 0x100) / 0x10);
#else
			" %d.%01d\n", __BORLANDC__ / 0x100,
			(__BORLANDC__ % 0x100) / 0x10);
#endif				/* __cplusplus */
#endif				/* __BORLANDC__ */
/************************************************************** Borland C */
...
void mwAbort(void)
{
	...
	while (mwHead != NULL && errors < 3) {
		...
		if (!(mwHead->flag & MW_NML)) {
			...
			mwWrite("\n\runfreed: <%ld> %s(%d), %ld bytes at %p \n",
				mwHead->count, mwHead->file, mwHead->line,
				(long)mwHead->size, data + mwOverflowZoneSize);
			if (mwCheckOF(data)) {
				mwWrite("[underflowed] ");
				FLUSH();
			}
			...
			if (mwTestMem(data, mwHead->size, MW_VAL_NML)) {
				...
				mwWrite
				    ("wild pointer: <%ld> NoMansLand %p alloc'd at %s(%d)\n",
				     mwHead->count, data + mwOverflowZoneSize,
				     mwHead->file, mwHead->line);
				FLUSH();
			}
			mwNmlNumAlloc--;
...
void *mwMark(void *p, const char *desc, const char *file, unsigned line)
{
	...
	for (mrk = mwFirstMark; mrk; mrk = mrk->next) {
		if (mrk->next != NULL
		    && !mwIsReadAddr(mrk->next, sizeof(mwMarker))) {
			mwWrite
...
void *mwMalloc(size_t size, const char *file, int line)
{
	...
	if (mwUseLimit && ((long)size + mwStatCurAlloc > mwAllocLimit)) {
		mwWrite("limit fail: <%ld> %s(%d), %ld wanted %ld available\n",
			mwCounter, file, line, (long)size,
			mwAllocLimit - mwStatCurAlloc);
		mwIncErr();
		FLUSH();
		MW_MUTEX_UNLOCK();
		return NULL;
...
void *mwRealloc(void *p, size_t size, const char *file, int line)
{
	...
	if (mwIsOwned(mw, file, line)) {
		...
		if (mw->flag & MW_NML) {
			mwIncErr();
			if (*
			    ((unsigned char *)(mw) + mwDataSize +
			     mwOverflowZoneSize) != MW_VAL_NML) {
				mwWrite
		...
		/* if this allocation would violate the limit, fail it */
		if (mwUseLimit
		    && ((long)size + mwStatCurAlloc - (long)mw->size >
			mwAllocLimit)) {
			TESTS(file, line);
			mwCounter++;
			mwWrite
...
void mwFree(void *p, const char *file, int line)
{
	...
	if (mwIsOwned(mw, file, line)) {
		...
		if (mwNML) {
			...
			memset(((char *)mw) + mwDataSize + mwOverflowZoneSize,
			       MW_VAL_NML, mw->size);
		} else {
			/* unlink the allocation, and enter the post-free data */
			mwUnlink(mw, file, line);
			memset(mw, MW_VAL_DEL,
			       mw->size + mwDataSize + mwOverflowZoneSize +
			       mwOverflowZoneSize);
			if (mwFBI) {
				memset(mw, '.',
				       mwDataSize + mwOverflowZoneSize);
				sprintf(buffer, "FBI<%ld>%s(%d)", mwCounter,
					file, line);
				strncpy((char *)(void *)mw, buffer,
					mwDataSize + mwOverflowZoneSize);
			}
			free(mw);
		}
...
int mwAssert(int exp, const char *exps, const char *fn, int ln)
{
	...
	if (mwAriFunction != NULL) {
		sprintf(buffer, "MEMWATCH: assert trap: %s(%d), %s", fn, ln,
			exps);
...
int mwVerify(int exp, const char *exps, const char *fn, int ln)
{
	...
	if (mwAriFunction != NULL) {
		sprintf(buffer, "MEMWATCH: verify trap: %s(%d), %s", fn, ln,
			exps);
		i = (*mwAriFunction) (buffer);
		if (i == 0) {
			mwWrite
...
static unsigned mwGrab_(unsigned kb, int type, int silent)
{
	...
	for (; kb; kb--) {
		if (mwUseLimit &&
		    (mwStatCurAlloc + mwGrabSize + (long)sizeof(mwGrabData) >
		     mwAllocLimit)) {
			if (!silent) {
				mwWrite
...
static int mwRelink(mwData * mw, const char *file, int line)
{
	...
	if (mwHead != mw) {
		...
		for (mw1 = mwHead; mw1; mw1 = mw1->next) {
			...
			if (mw1->next &&
			    (!mwIsSafeAddr(mw1->next, mwDataSize)
			     || mw1->next->prev != mw1)) {
				mwWrite
	...
	if (mwTail != mw) {
		...
		for (mw1 = mwTail; mw1; mw1 = mw1->prev) {
			...
			if (mw1->prev
			    && (!mwIsSafeAddr(mw1->prev, mwDataSize)
				|| mw1->prev->next != mw1)) {
				mwWrite
 ...
 emergency:
	...
	mw1 = NULL;
	if (mwHead != NULL) {
		if (!mwIsReadAddr(mwHead, mwDataSize)
		    || mwHead->check != CHKVAL(mwHead)) {
			mwWrite
						...
						mwWrite
						    ("relink: forward chain's first damaged MW is MW-%p, %ld %sbytes at %s(%d)\n",
						     ...
						     (mw->
						      flag & MW_NML) ?
						     "NoMansLand " : "",
						     mwIsReadAddr(mw1->file,
								  16) ? mw1->
						     file : "<garbage-pointer>",
						     mw1->line);
					} else {
						mwWrite
 ...
 scan_reverse:
	mw2 = NULL;
	if (mwTail != NULL) {
		if (!mwIsReadAddr(mwTail, mwDataSize)
		    || mwTail->check != CHKVAL(mwTail)) {
			mwWrite
 ...
 verifyok:
	...
	if (count == mwNumCurAlloc) {
		...
		mwWrite("relink: partial, %ld MW-blocks of %ld bytes lost\n",
			mwNmlNumAlloc + mwNumCurAlloc - count,
			mwNmlCurAlloc + mwStatCurAlloc - size);
		return 0;
	}
	...
	if (mw->prev && mw->prev->next != mw) {
		mwWrite
		    ("internal: <%ld> %s(%d), buffer <%ld> %s(%d) link1 broken\n",
		     mwCounter, file, line, (long)mw->size, mw->count, mw->file,
		     mw->line);
		mwIncErr();
		if (!mwRelink(mw, file, line))
			retv = 2;
	...
	while (mw != NULL) {
		if (!(mw->flag & MW_NML))
			...
			if (mwTestMem(data, mw->size, MW_VAL_NML)) {
				...
				mwWrite
				    ("wild pointer: <%ld> NoMansLand %p alloc'd at %s(%d)\n",
				     mw->count, data + mwOverflowZoneSize,
				     mw->file, mw->line);
			}
			mw2 = mw->next;
