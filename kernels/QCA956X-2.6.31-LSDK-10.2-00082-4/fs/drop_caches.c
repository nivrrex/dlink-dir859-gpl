	...
	struct file *file, void __user *buffer, size_t *length, loff_t *ppos)
{
	...
	if (write) 
	{
        if ((cache_cleanup_timer.function == NULL) && (sysctl_drop_caches == 1))
        {
            init_timer(&cache_cleanup_timer);
            INIT_WORK(&drop_cache_workq, (void *)drop_caches);
            cache_cleanup_timer.expires = (jiffies + CACHE_CLEANUP_TIMEOUT);
        ...
        else if((cache_cleanup_timer.function != NULL) && (sysctl_drop_caches == 0))
        {
        	del_timer(&cache_cleanup_timer);
			cache_cleanup_timer.function = NULL;
			printk("**** %s: all done timer removed ...**** \n", __func__);
        }
	}
#if 0
	if (write) {
