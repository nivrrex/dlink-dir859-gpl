...
connmark_mt(const struct sk_buff *skb, const struct xt_match_param *par)
{
    ...
    return ((ct->mark2 & info->mask) == info->mark) ^ info->invert;             //info->linux/netfilter/xt_connmark2.h, ct->net/netfilter/nf_conntrack.h
}

static bool
connmark_mt_v0(const struct sk_buff *skb, const struct xt_match_param *par)
{
    const struct xt_connmark2_info *info = par->matchinfo;
    const struct nf_conn *ct;
    enum ip_conntrack_info ctinfo;
    ...
    return ((ct->mark2 & info->mask) == info->mark) ^ info->invert;
}

static bool connmark_mt_check_v0(const struct xt_mtchk_param *par)
{
    const struct xt_connmark2_info *cm = par->matchinfo;
...
#ifdef CONFIG_COMPAT
struct compat_xt_connmark2_info {
...
{
    const struct compat_xt_connmark2_info *cm = src;
    struct xt_connmark2_info m = {
        .mark   = cm->mark,
        .mask   = cm->mask,
        .invert = cm->invert,
    };
...
{
    const struct xt_connmark2_info *m = src;
    struct compat_xt_connmark2_info cm = {
        .mark   = m->mark,
        .mask   = m->mask,
        .invert = m->invert,
    };
...
static struct xt_match connmark_mt_reg[] __read_mostly = {
    {
        ...
        .matchsize  = sizeof(struct xt_connmark2_info),
#ifdef CONFIG_COMPAT
        .compatsize = sizeof(struct compat_xt_connmark2_info),
