...
void get_pre_packets(struct nf_conn *ct, int dir, int dirx)
{
    struct nf_conn_counter *acct;
    acct = nf_conn_acct_find(ct);
    if (!acct)
        return;
    ct->pre_tx = (unsigned long long)acct[dir].packets;
    ct->pre_rx = (unsigned long long)acct[dirx].packets;
    return;
}
EXPORT_SYMBOL_GPL(get_pre_packets);
#endif
#ifdef CONFIG_NF_CONNTRACK_PARENTAL
u_int64_t get_packets(struct nf_conn *ct, int dir)
{
	struct nf_conn_counter *acct;
	acct = nf_conn_acct_find(ct);
	if (!acct)
		return 0;
	return (unsigned long long)acct[dir].packets;
}
EXPORT_SYMBOL_GPL(get_packets);
#endif
