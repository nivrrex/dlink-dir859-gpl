...
void (*nf_ct_qca_nat_hook)(struct nf_conn *ct, unsigned int action);
EXPORT_SYMBOL_GPL(nf_ct_qca_nat_hook);
#endif
								...
								struct net *net)
{
	...
	{
		if (hlist_nulls_empty(&net->ct.hash[index]))
			continue;
		else 
		{
			struct hlist_nulls_node *n;
...
void (*nf_ct_close_session)(struct nf_conn *ct);

void set_close_session(void *close_session)
{
	nf_ct_close_session = close_session;
}
EXPORT_SYMBOL_GPL(set_close_session);
#endif
...
void nf_conntrack_free(struct nf_conn *ct)
{
	struct net *net = nf_ct_net(ct);
#ifdef CONFIG_NF_CONNTRACK_PARENTAL
	if(nf_ct_close_session != NULL)
		nf_ct_close_session(ct);
#endif
...
#ifdef CONFIG_NF_CONNTRACK_MARK2
		ct->mark2 = exp->master->mark2;
#endif
