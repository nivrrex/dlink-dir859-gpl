...
#if defined(CONFIG_NF_CONNTRACK_ACCT)
    if (seq_printf(s, "pre_tx=%llu ", ct->pre_tx))
        goto release;
#endif
...
#if defined(CONFIG_NF_CONNTRACK_ACCT)
    if (seq_printf(s, "pre_rx=%llu ", ct->pre_rx))
        goto release;
#endif
...
#if defined(CONFIG_NF_CONNTRACK_MARK2)
    if (seq_printf(s, "mark2=%u ", ct->mark2))
	        goto release;
#endif
...
#if defined(CONFIG_NF_CONNTRACK_MARK2)
enum{PROTO_NUM,IP,PORT,STATUS,CONNMARK};
#else
...
#if defined(CONFIG_NF_CONNTRACK_MARK2)
	if( !(para->port==0 && para->proto_num==0 && para->ip==0 && para->status==0 && para->connmark ==0) )
#else
	if( !(para->port==0 && para->proto_num==0 && para->ip==0 && para->status==0) )
#endif
	{
#if defined(CONFIG_NF_CONNTRACK_MARK2)
		if( match(i,PROTO_NUM,para->proto_num) && match(i,IP,para->ip) && match(i,PORT,para->port) && match(i,STATUS,para->status) && match(i,CONNMARK, para->connmark))
#else
if( match(i,PROTO_NUM,para->proto_num) && match(i,IP,para->ip) && match(i,PORT,para->port) && match(i,STATUS,para->status))
#endif
...
#if defined(CONFIG_NF_CONNTRACK_MARK2)
int parse_connmark(char *buffer, struct parameter *para)
{
    int offset = 0;
	...
	para->connmark = simple_strtol(buffer, NULL, 10);
	return offset;
}
#endif
...
#if defined(CONFIG_NF_CONNTRACK_MARK2)
	tmp = strstr(buffer + offset, "--connmark");
	    if (tmp != NULL) {
			offset += parse_connmark(tmp + 11, para);
	}
#endif
}
...
#if defined(CONFIG_NF_CONNTRACK_MARK2)
	str+=sprintf(str,"echo -n \"--proto TCP --port 12 --status UNREPLIED --ip 192.168.0.100 --connmark 22\" > /proc/nf_conntrack_flush\n");
#else
...
#ifdef CONFIG_NF_CONNTRACK_PARENTAL
static void *net_custom;

void * get_net_custom()
{
	return net_custom;
}
EXPORT_SYMBOL_GPL(get_net_custom);
int set_net_custom(struct net *net)
	{
		net_custom = (void *)((struct net *) net);
	}
EXPORT_SYMBOL_GPL(set_net_custom);
#endif
