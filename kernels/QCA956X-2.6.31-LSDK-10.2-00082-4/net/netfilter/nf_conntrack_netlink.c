...
static inline int
ctnetlink_dump_mark2(struct sk_buff *skb, const struct nf_conn *ct)
{
    NLA_PUT_BE32(skb, CTA_MARK2, htonl(ct->mark2));
    return 0;
...
#else
#define ctnetlink_dump_mark2(a, b) (0)
#endif
...
#endif
#ifdef CONFIG_NF_CONNTRACK_MARK2
           + nla_total_size(sizeof(u_int32_t)) /* CTA_MARK2 */
#endif
...
#ifdef CONFIG_NF_CONNTRACK_MARK2
    if ((events & (1 << IPCT_MARK) || ct->mark2)
        && ctnetlink_dump_mark2(skb, ct) < 0)
        goto nla_put_failure;
#endif
...
ctnetlink_change_conntrack(struct nf_conn *ct, struct nlattr *cda[])
{
    ...
    if (cda[CTA_MARK2])
    ct->mark2 = ntohl(nla_get_be32(cda[CTA_MARK2]));
#endif
