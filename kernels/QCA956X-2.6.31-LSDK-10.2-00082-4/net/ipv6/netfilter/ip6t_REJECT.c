...
static inline void
send_parmprob(struct net *net, struct sk_buff *skb_in, unsigned char code,
	     unsigned int hooknum)
{
	if (hooknum == NF_INET_LOCAL_OUT && skb_in->dev == NULL)
		skb_in->dev = net->loopback_dev;

	icmpv6_send(skb_in, ICMPV6_PARAMPROB, code, 0, NULL);
}
...
reject_tg6(struct sk_buff *skb, const struct xt_target_param *par)
{
	...
	struct net *net = dev_net((par->in != NULL) ? par->in : par->out);
		...
		.fc_metric      = IP6_RT_PRIO_USER, //IP6_RT_PRIO_KERN, 
	...
	case IP6T_TCP_RESET:
		send_reset(net, skb);
		break;
	case IP6T_ICMP6_SRCADDR_FAILED:
		ipv6_addr_copy(&cfg.fc_dst, &ipv6_hdr(skb)->saddr); 
		ip6_route_add(&cfg);
		send_unreach(net, skb, ICMPV6_SRC_ADDR_FAILED, par->hooknum);
		break;
	case IP6T_ICMP6_HDR_FIELD:
		send_parmprob(net, skb, ICMPV6_HDR_FIELD, par->hooknum);
		break;
	default:
		if (net_ratelimit())
