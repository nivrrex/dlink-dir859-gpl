...
/*
$dev			device name
$vlan_id		vlan setting id
$internetid 	internet id,if vlan id != internet id, then bridge to wan
*/
...
{
	startcmd('echo 1 > /proc/rtk_vlan_support');//enable vlan
	...
	//set the same mac address for lan
...
{
	//Some model can't echo 0 > /proc/rtk_vlan_support,it might block pppoe's connection
	startcmd('ifconfig eth0 down');
	startcmd('echo 0 > /proc/rtk_vlan_support');//disable vlan
	startcmd('ifconfig eth0 up');
	/*
	$vlan_proc = fread("", "/proc/rtk_vlan_support");
	if($vlan_proc == "rtk_vlan_support_enable: 1")
	{
		TRACE_debug("set vlan to :".$vlan);
		startcmd('echo 0 > /proc/rtk_vlan_support');
	}
	*/
}

stopcmd('echo 0 > /proc/rtk_vlan_support');//disable vlan
