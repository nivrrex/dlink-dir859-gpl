		...
		if (Total_MacFilterList >= 1)
		{
			for (var j = 0; j < Total_Device; j ++)
			{
				...
				for (var k = 1; k <= Total_MacFilterList; k ++)
				{
					...
					var tmpSCH = result_xml.Get("GetMultipleHNAPsResponse/GetMACFilters2Response/MACList/MACInfo:" + k + "/ScheduleName");
					if (array2Dv[j][1] == tmpMAC)
					{
						checkMac = 1;
					}
					...
					if (checkMac == 1)
					{
						if (tmpSCH != "" || tmpSCH.length != 0)
						{
							...
							{
								var sn = tmpXML_ScheduleList.Get("GetScheduleSettingsResponse/ScheduleInfoLists:" + n + "/ScheduleName");
								if (tmpSCH == sn)
								{
									checkSN = 1;
	...
	if (Get_ClientStatus != "link_IconX_Block")
	{
		...
		else
		{
			...
			if (Get_ClientSchedules != "" || Get_ClientSchedules != null)
			{
				...
				{
					if(schedule.options[i].text === Get_ClientSchedules)
					{
...
function closeEditPOP()	{	HNAP.GetXMLAsync("GetClientInfo", null, "GetValue", function(xml)	{	GetResult_GCI_1(xml)	});	}
	...
	if (GetResult_GCI_3 == "OK")
	{
		...
		if (GetMacFilterListNumber > 0)
		{
			...
			for (var i = 1; i <= GetMacFilterListNumber; i ++)
			{
				...
				array2DvMF[CheckCount][2] = result_xml.Get("GetMACFilters2Response/MACList/MACInfo:" + i + "/ScheduleName");
				CheckCount ++;
			}
		}
		if (GetMacFilterListNumber == 0 && document.getElementById("TimmyCheckBox").checked == true)
		{
			...
			if (Set_Client_ScheduleName != "ALWAYS")	{	xml_SetMacFilter.Set("SetMACFilters2/MACList/MACInfo:1/ScheduleName", Set_Client_ScheduleName);	}
		...
		else if (GetMacFilterListNumber >= 1 && document.getElementById("TimmyCheckBox").checked == true)
		{
			...
			{
				if (Set_Client_MACAddress == array2DvMF[i][1])	{	targetIndex = i;	break;}
				else	{	targetIndex ++;	}
			}
			...
			xml_SetMacFilter.Set("SetMACFilters2/Enabled", "true");
			xml_SetMacFilter.Set("SetMACFilters2/IsAllowList", "false");
			if (targetIndex < GetMacFilterListNumber && ScheduleCheck == 1)
			{
				for (var j = 0;	j < GetMacFilterListNumber; j ++)
				{
					if (targetIndex == j)
					{
						...
						if (Set_Client_ScheduleName != "ALWAYS")	{	xml_SetMacFilter.Set("SetMACFilters2/MACList/MACInfo:" + SetCounter + "/ScheduleName", Set_Client_ScheduleName);	}
					...
					else
					{
						...
						xml_SetMacFilter.Set("SetMACFilters2/MACList/MACInfo:" + SetCounter + "/ScheduleName", array2DvMF[SetCounter - 1][2]);
					}
					SetCounter ++;
				}
			}
		...
		else if (GetMacFilterListNumber > 1 && document.getElementById("TimmyCheckBox").checked == false)
		{
			...
			if (targetIndex < GetMacFilterListNumber-1)
			{
				...
				for (var k = 0 ; k < GetMacFilterListNumber-1; k ++)
				{
					...
					xml_SetMacFilter.Set("SetMACFilters2/MACList/MACInfo:" + SetCounter + "/ScheduleName", array2DvMF_a[k][2]);
					SetCounter ++;
				}
		...
		else if (GetMacFilterListNumber == 1 && document.getElementById("TimmyCheckBox").checked == false)
		{
			...
			if (Competence == 1000)
			{
				...
				xml_SetMacFilter.Set("SetMACFilters2/MACList/MACInfo/ScheduleName", "");
				HNAP.SetXMLAsync("SetMACFilters2", xml_SetMacFilter, function(xml)	{	GetResult_GCI_4(xml);	});
			}
			else	{	ClearAndRenew();	}
		}
