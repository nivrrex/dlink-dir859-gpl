...
var STATUS = 'status_'
var NAME = 'name_';
var DESTINATION_IP = 'destinationIP_';
var NETMASK = 'netmask_';
...
function GetHNAPInformation(Status, Name, DestinationIP, Netmask, Gateway, Matrix, Interface)
{	
	a = Status;
	b = Name;
	c = DestinationIP;
...
function addRowToTable(num)
{
	if (IsLoaded)
	{
		...
		Input3.setAttribute('id', DESTINATION_IP + iteration);
		...
		Input4.setAttribute('id', NETMASK + iteration);
...
function AddRowToIndex()
{
	...
	c = document.getElementById("sr_DestinationIP").value;
	d = document.getElementById("sr_Netmask").value;
...
function editDataGet(id)
{
	...
	var tmp_editDestinationIP = tbl.rows[id].childNodes[2].childNodes[0].innerHTML;
	var tmp_editNetmask = tbl.rows[id].childNodes[3].childNodes[0].innerHTML;
	...
	document.getElementById("sr_EditDestinationIP").value = tmp_editDestinationIP;
	document.getElementById("sr_EditNetmask").value = tmp_editNetmask;
	...
	tmp_DestinationIP = tmp_editDestinationIP;
	tmp_Netmask = tmp_editNetmask;
...
function assignRowToIndex(id)
{
	...
	var get_EditLocalIP = document.getElementById("sr_EditDestinationIP").value;
	var get_EditNetmask = document.getElementById("sr_EditNetmask").value;
	...
	if (get_EditName == "" || get_EditLocalIP == "" || get_EditNetmask == "" || get_EditGateway == "" || get_EditMatrix == "")
	{
	...
	else
	{
		...
		tbl.rows[id].childNodes[2].childNodes[0].innerHTML = tmp_DestinationIP;
		tbl.rows[id].childNodes[3].childNodes[0].innerHTML = tmp_Netmask;
	...
	if (IsLoaded)
	{
		if (tbl.tBodies[0].rows[startingIndex])
		{
			...
			for (var i = startingIndex; i < tbl.tBodies[0].rows.length; i ++)
			{
				...
				tbl.tBodies[0].rows[i].myRow.three.id = DESTINATION_IP + count;
				tbl.tBodies[0].rows[i].myRow.four.id = NETMASK + count;
