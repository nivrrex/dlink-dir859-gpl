...
function repeater_parameters($uid)
{
        $p = XNODE_getpathbytarget("", "phyinf", "uid", $uid, 0);
        if($p == "") return 1;
        ...
        if($wifi == "") return 1;
        anchor($wifi);
        if (query("opmode") == "REPEATER" || query("opmode") == "APC" )
        {
        	if (query($p."/active") == "1")
		{
			$APClienable=0;
			...
			if($devconfsize > 0){$APClienable=1;}
			echo "MACRepeaterEn=1\n";
			echo "ApCliEnable=".$APClienable."\n";
			echo "ApCliSsid=".              $ssid. "\n";
			echo "ApCliBssid=".             $bssid. "\n";
			echo "ApCliAuthMode=".  $auth       ."\n";
			echo "ApCliEncrypType=".$encrypt       ."\n";
			if ($encrypt == "WEP")
			{
			...
			} else if ($encrypt == "TKIP" || $encrypt == "AES")
			{
					echo "ApCliWPAPSK=".query("nwkey/psk/key")."\n";
			}
			$wpshelper = "/etc/scripts/wps_sta.sh";
			$xmldbhelper = "/etc/scripts/wps_sta_stat.sh";
			echo "AlphaWpsHelper=".$wpshelper."\n";
			echo "AlphaStateHelper=".$xmldbhelper."\n";
		}
	}
}
/**********************************************************************************/
...
$PHY_UID="BAND24G-1.2";
$phy = XNODE_getpathbytarget("", "phyinf", "uid", $PHY_UID);
$guset_zones = query($phy."/active");
if($guset_zones == "1")
{
	$disable_routing_zones = query("/acl/firewall2/entry:1/enable");
	$disable_isolation_gzone = query("/acl/firewall2/entry:2/enable");
	
	if($disable_isolation_gzone == "1")
	{
...
repeater_parameters("BAND24G-1.3");
/* -------- RT2860AP.dat end -------*/
?>
