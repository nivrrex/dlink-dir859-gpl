...
Datalist.prototype.editData = function(id, newdata){
	...
	this.list[rowNum].Band = newdata.Band;//Add for wifi band
	this.list[rowNum].SignalLevel = newdata.SignalLevel;//Add for wifi signal strength
...
//constructor
function Data(macAddress, ipv4Address, ipv6Address, type, deviceName, nickName, reserveIP, Band, signalLevel){
	...
	this.Band = Band;//add for wifi band
	this.SignalLevel = signalLevel;//add for wifi signal strength
...
Data.prototype = 
{
	...
	showClientInfo: function()
	{
		...
		var Band = self.Band;	//default band is empty
		var macFilterStatus = "";
		var signalLevel = "";
		//Add tag
		if(self.Type == "New")
		...
		{
			macFilterStatus = "client_Access_enable";
			macFilterString = I18N("j", "Enabled");
			fontColor = "#FF0000";
			if((self.Type == "Blocked")||(self.Status))
			{
				if(objType == "WIFI_2.4G" || objType == "WIFI_2.4G_GUEST" || objType == "WIFI_5G" || objType == "WIFI_5G_GUEST")	{
					cssBackGround = "link_IconW_Block";
				}
				else {
					cssBackGround += "Block";
				}
		...
		{
			macFilterStatus = "client_Access";
			macFilterString = I18N("j", "Disabled");
			fontColor = "#000000";
			cssBackGround += "Allow";
		}

		if(self.SignalLevel == "low") {
			signalLevel = "client_Signal_Lv1";
		} else if(self.SignalLevel == "mid") {
			signalLevel = "client_Signal_Lv2";
		} else if(self.SignalLevel == "high") {
			signalLevel = "client_Signal_Lv3";
		} else if(self.SignalLevel == "high+") {
			signalLevel = "client_Signal_Lv4";
		}
		...
		var outputString = "<td>"
			   ...
			   + '<div class ="' + macFilterStatus + '"></div>'
			   + '<div class ="' + signalLevel + '"></div>'
...
function Get_ClientInfo()
{
	...
	$.when(clientinfo_query, macfilter_query).done(function(obj)
	{
		...
		for(var row=datalist.list.length-1;row >= 0;row--)
		{
		...
		{
			var client = clientInfo.ClientInfoLists.ClientInfo[clientUnit];
			var data = new Data(client.MacAddress, client.IPv4Address, client.IPv6Address, client.Type, client.DeviceName, decode_char_text(client.NickName), client.ReserveIP, client.Band, client.SignalLevel);
...
function addData()
{
	...
	document.getElementById("IPAdrReserve_Info").style.display = "none";
...
function editData(id)
{
	...
	else
	{
		...
		if (data.ScheduleName != "")
		{
			...
			{
				if(schedule.options[i].value === decode_char_text(data.ScheduleName))
				{
...
function closeEditPOP(id)
{
	...
	var data = datalist.getData(id);
	//data.NickName = encode_char_text(name);
	...
	client.NickName = name==data.DeviceName?"":name;
...
function closeAddPOP()
{
	...
	{
		if(!COMM_ValidV4Format(reserveIP) || !COMM_ValidV4Addr(reserveIP)){
			document.getElementById("IPAdrReserve_Info").style.display = "";
			return;
		}
		for(var row in datalist.list)
		{
			var obj = datalist.list[row];
			if(obj.IPv4Address == reserveIP)
			{
				document.getElementById("IPAdrReserve_Info").style.display = "";
				return;
			}
