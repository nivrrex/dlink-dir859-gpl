...
function wifi_repeater_service($wifi_uid,$prefix)
{
	...
	if ($a==1)
	{
		$ssid=getssid($wifi_uid,$dev);
		multi_ap_profile($dev,"service",$ssid,$authtype,$encrtype,$psk,$wep_key_1,$wep_key_len,$ascii);
		/*
		setssid($wifi_uid,$winfname);
	...
	*/
	...
	{
		multi_ap_profile($winfname,"service");
	}
	*/
...
//	fwrite("a",$_GLOBALS["STOP"], "killall hostapd > /dev/null 2>&1;\n");
	startcmd("xmldbc -k \"mdnsresponder\"");
	...
	{
		/* +++ upwifistats */
		startcmd("xmldbc -P /etc/services/WIFI/updatewifistats.php -V PHY_UID=".$prefix."-REPEATER > /var/run/restart_upwifistats.sh");
		startcmd("sh /var/run/restart_upwifistats.sh");
		stopcmd("xmldbc -P /etc/services/WIFI/updatewifistats.php -V PHY_UID=".$prefix."-REPEATER > /var/run/restart_upwifistats.sh;");
		stopcmd("sh /var/run/restart_upwifistats.sh");
		/* --- upwifistats */
		/* +++ wscd */
		if ($prefix == "BAND24G"){
		...
		if($uid=="BAND5G-REPEATER")
		{
			//startcmd("sleep 1");
			startcmd("usockc /var/wanmonitor_ctrl WISP_READY");
		}
	}
	startcmd('xmldbc -t "mdnsresponder:3:service MDNSRESPONDER restart"'); 
	return error(0);
}
