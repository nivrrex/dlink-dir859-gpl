...
function bwc_tc_spq_2013gui_start($rtbwcp, $name, $ifname)
{
	...
	TRACE_info("[Vic_Debug]: total rate = ".$trate);
	...
	foreach($rtbwcp."/rules/entry")
	{
		if (query("enable")=="1")
		{
			...
			/* priority */
			$bwcqdp_priority = query($bwcqdp."/priority");
			if		 ($bwcqdp_priority == "VO") { $mark_base_u = "1"; $mark_base_d = "1"; $hw_queue_no_u = "1"; $hw_queue_no_d = "5";}          //Highest    
			else if($bwcqdp_priority == "VI") { $mark_base_u = "2"; $mark_base_d = "2"; $hw_queue_no_u = "2"; $hw_queue_no_d = "6";}          //Higher     
			else if($bwcqdp_priority == "BG") { $mark_base_u = "3"; $mark_base_d = "3"; $hw_queue_no_u = "3"; $hw_queue_no_d = "7";}          //Normal     
			else if($bwcqdp_priority == "BE") { $mark_base_u = "4"; $mark_base_d = "4"; $hw_queue_no_u = "4"; $hw_queue_no_d = "8";} 			//Best Effort
			else { startcmd("echo bwcsvcs.php: Unknown Traffic Control priority...ERROR!!!"); }		
			
			if( $name == "WAN-1" )   /* Upload bandwidth control */
			{
				startcmd("#=========================================================================================");
				startcmd($ipt_add_prefix." -m iprange --src-range ".$startip."-".$endip." -j MARK --set-mark ".$mark_base_u);
				startcmd($ipt_add_prefix." -m iprange --src-range ".$startip."-".$endip." -j RETURN");
			}
			if( $name == "LAN-1" ) /* Download bandwidth control */
			{
				startcmd("#=========================================================================================");
				startcmd($ipt_add_prefix." -m iprange --dst-range ".$startip."-".$endip." -j MARK --set-mark ".$mark_base_d);
				startcmd($ipt_add_prefix." -m iprange --dst-range ".$startip."-".$endip." -j RETURN");	
			}
		}
	}	

	/* setup hw qos for MT7621 */
	//if($trate < 1000000)
		$trate = 1000000;

	if($prefix == "WAN") {
		$linkstatus = get("", PHYINF_getruntimephypath($name)."/linkstatus");
		if($linkstatus=="10F") $trate = 10000; //Kbps for unit.
		else if ($linkstatus=="100F") $trate = 100000;
		else if ($linkstatus=="1000F") $trate = 1000000;
		else $trate = 1000000;
	}
	//TRACE_info("[Vic_Debug]: adjust rate = ".$trate);
	
	$prio_VO = $trate*50/100;
	$prio_VI = $trate*30/100;
	$prio_BG = $trate*10/100;
	$prio_BE = $trate*10/100;

	if( $name == "LAN-1" )
	{
		TRACE_info("Init HW QoS Setting......");
		startcmd("qdma sch_rate 0 0 0");
		startcmd("qdma sch_rate 1 0 0");	
	
		$q_idx = 0;
		while($q_idx < 16)
		{
			if($q_idx != 0 && $q_idx != 1 && $q_idx != 8 && $q_idx != 9)
			{
				startcmd("qdma resv ".$q_idx." 0 0");
			}
			
			if($q_idx < 8)
			{
				startcmd("qdma sch ".$q_idx." 0");
			}
			else
			{
				startcmd("qdma sch ".$q_idx." 1");
			}
				
			startcmd("qdma rate ".$q_idx." 0 0 0 0");
			//startcmd("qdma weight ".$q_idx." ".$q_idx);
			$q_idx++;
		}
	...
	if( $name == "WAN-1" )   // Upload bandwidth control
	{
		startcmd("qdma sch_rate 0 1 ".$trate); 	//set sch0 and enable control rate

		$sch_no = "0";
		startcmd("qdma resv 2 10 10");
		startcmd("qdma sch 2 ".$sch_no);
		startcmd("qdma rate 2 1 ".$prio_BE." 1 ".$trate);
		startcmd("qdma m2q 4 2");		

		startcmd("qdma resv 3 20 20");
		startcmd("qdma sch 3 ".$sch_no);
		startcmd("qdma rate 3 1 ".$prio_BG." 1 ".$trate);
		startcmd("qdma m2q 3 3");	

		startcmd("qdma resv 4 30 30");
		startcmd("qdma sch 4 ".$sch_no);
		startcmd("qdma rate 4 1 ".$prio_VI." 1 ".$trate);
		startcmd("qdma m2q 2 4");	

		startcmd("qdma resv 5 40 40");
		startcmd("qdma sch 5 ".$sch_no);
		startcmd("qdma rate 5 1 ".$prio_VO." 1 ".$trate);
		startcmd("qdma m2q 1 5");			
	}
		
	if( $name == "LAN-1" ) // Download bandwidth control
	{
		startcmd("qdma sch_rate 1 1 ".$trate); 	//set sch1 and enable control rate

		$sch_no = "1";
		startcmd("qdma resv 10 10 10");
		startcmd("qdma sch 10 ".$sch_no);
		startcmd("qdma rate 10 1 ".$prio_BE." 1 ".$trate);
		startcmd("qdma m2q 4 10");
		
		startcmd("qdma resv 11 20 20");
		startcmd("qdma sch 11 ".$sch_no);
		startcmd("qdma rate 11 1 ".$prio_BG." 1 ".$trate);
		startcmd("qdma m2q 3 11");
		
		startcmd("qdma resv 12 30 30");
		startcmd("qdma sch 12 ".$sch_no);
		startcmd("qdma rate 12 1 ".$prio_VI." 1 ".$trate);
		startcmd("qdma m2q 2 12");

		startcmd("qdma resv 13 40 40");
		startcmd("qdma sch 13 ".$sch_no);
		startcmd("qdma rate 13 1 ".$prio_VO." 1 ".$trate);
		startcmd("qdma m2q 1 13");
	}
}
...
function bwc_setup($name)
{
    ...
    stopcmd("echo ".$name." Stop Traffic Control system: TC_SPQ_2013GUI ...");
		...
		if( query($bwcp."/enable") == "1")
		{
			//startcmd("service HW_NAT restart"); // Stop ralink HW_NAT
