...
{
	$ValueUrlEncode = urlencode("m",$value);
...
function mdb_get($cmd_name,$WAN1,$WLAN1)
{
	...
	else if($cmd_name == "hw_version")	{show_result(UrlEncode(query("/runtime/device/hardwareversion")));}
...
function mdb_set($cmd_name,$arg)
{
	...
	if($cmd_name != "")
	{
		...
		if($found == 0)
		{
			...
			set($mdb_tmp_path."/entry:".$next_ID."/ret",$ret);
		}
	}
	if ($ret == "0")
		show_result($arg);
