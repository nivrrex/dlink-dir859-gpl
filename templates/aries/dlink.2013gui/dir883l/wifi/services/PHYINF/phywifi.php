...
function devname($uid)
{
	...
	else if ($uid=="STATION24G-1.1")
		return $_GLOBALS["BAND24G_STA_DEVNAME"];
	else if ($uid=="STATION5G-1.1")
		return $_GLOBALS["BAND5G_STA_DEVNAME"];
	else if ($uid=="WIFI-REPEATER")
		return $_GLOBALS["WIFI_REP_DEVNAME"];
	return "";
}
...
function wifi_AP($is5G)
{
	...
	while ($i>0)
	{
		...
		else
		{
			...
			$ssid 			= get("s", $wifi."/ssid");		if ($ssid=="")	{$ssid="dlink";} 	
			$wlan_partition =		query($wifi."/acl/isolation");	if ($wlan_partition!="1")	{$wlan_partition="0";}
			...
			$puren			= "0";
			$pureg			= "0";
			$rate=query("dot11n/mcs/index");
			if (query("dot11n/mcs/auto") != "1")	
			{
				if($rate > 15){
					$rate -= 16;
					$rate =  "0x9".$rate."9".$rate."9".$rate."9".$rate;
				}
				else{
					$rate =  dec2strf("%x",$rate);
					$rate =  "0x8".$rate."8".$rate."8".$rate."8".$rate;
				}
			}
			else	{$rate = "auto";}
			$txpower		= query("txpower"); if ($txpower=="")	{$txpower="100";}
			...
			$params = $params."RATECTL=".$rate.";";
			$params = $params."WLAN_PARTITION=".$wlan_partition.";";
			$params = $params."TXPOWER=".$txpower.";";
...
function wifi_STA($is5G)
{
	...
	if ($p != "")
	{
		...
		else
		{
			...
			if ($bandmode == "2G")
			{
				$makeVAPcmd = "/etc/ath/makeVAP sta \"My Net N600 sta\" \"BANDMODE=".$bandmode.";CH_MODE=11NGHT20;PUREN=0;PUREG=0;RF=RF;PRI_CH=0;ATH_NAME=".$dev.";R_SHORTGI=1;\"";
			}
			else if ($bandmode == "5G")
			{
				$makeVAPcmd = "/etc/ath/makeVAP sta \"My Net N600 sta\" \"BANDMODE=".$bandmode.";CH_MODE=11NAHT20;PUREN=0;PUREG=0;RF=RF;PRI_CH=0;ATH_NAME=".$dev.";R_SHORTGI=1;\"";
			}
			//TRACE_error("makeVAPcmd=".$makeVAPcmd);
...
function wifi_REP($is5G)
{
	...
	if ($p != "")
	{
		...
		else
		{
			...
			if ($bandmode == "2G")
			{
				$makeVAPcmd = "/etc/ath/makeVAP sta-ext \"".$gw_name." sta\" \"BANDMODE=".$bandmode.";CH_MODE=11NGHT20;PUREN=0;PUREG=0;RF=RF;PRI_CH=0;ATH_NAME=".$dev.";R_SHORTGI=1;\"";
			}
			else if ($bandmode == "5G")
			{
				$makeVAPcmd = "/etc/ath/makeVAP sta-ext \"".$gw_name." sta\" \"BANDMODE=".$bandmode.";CH_MODE=11NAHT20;PUREN=0;PUREG=0;RF=RF;PRI_CH=0;ATH_NAME=".$dev.";R_SHORTGI=1;\"";
			}
			//TRACE_error("makeVAPcmd=".$makeVAPcmd);
...
function wificonfig($uid)
{
	...
	if($is_APmode == "AP") {wifi_AP($is_5G);}
	else if($is_APmode == "STA") {wifi_STA($is_5G);}
	else {wifi_REP($is_5G);}
