...
function generate_configs($phyinfuid, $output)
{
	...
	*/
	...
	else if ($encrtype == "WEP")
	{
		...
		$index2 = $index - 1;
		$wep_ascii = query($wifi."/nwkey/wep/ascii");
    fwrite("a", $output, '	key_mgmt=NONE\n');
    if($wep_ascii=="1") {	fwrite("a", $output, '	wep_key'.$index2.'="'.query($wifi."/nwkey/wep/key:".$index).'"\n'.);}
    else 								{	fwrite("a", $output, '	wep_key'.$index2.'='.query($wifi."/nwkey/wep/key:".$index).'\n'.);}
		
		fwrite("a", $output, '	wep_tx_keyidx='.$index.'\n');	    
	}
	else
	{
		$pskkey 	= query("nwkey/psk/passphrase");
		
		fwrite("a", $output, '	key_mgmt=WPA-PSK\n');
		
		if($pskkey=="1")	{	fwrite("a", $output, '	psk=\"'.query($wifi."/nwkey/psk/key").'\"\n');}
		else							{	fwrite("a", $output, '	psk='.query($wifi."/nwkey/psk/key").'\n');}
		
		if ($encrtype == "AES") 			{	fwrite("a", $output, '	pairwise=CCMP\n');}
		else if ($encrtype == "TKIP")	{	fwrite("a", $output, '	pairwise=TKIP\n');}
		else													{	fwrite("a", $output, '	pairwise=TKIP CCMP\n');}
	}
...
foreach ("/runtime/phyinf")
{
	...
	if (query("uid") !="WIFI-REPEATER") continue;
