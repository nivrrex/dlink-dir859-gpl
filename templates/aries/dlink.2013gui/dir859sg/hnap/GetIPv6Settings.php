...
function PPPBtnSetup($rwan, $wancable_status, $str_Connected, $str_Disconnected)
{
	...
	else if($connStat == "disconnected" || $connStat == "")
	{
		$str_networkstatus = $str_Disconnected;
		$wancable_status=0;
	}
	//else	$str_networkstatus = "Busy ...";	//follow HNAT spec
	else	$str_networkstatus = "Connecting";
...
{		
	...
	else if(query($dhcps6p."/mode")=="STATEFUL")
	{
		...
		if(query(INET_getpathbyinf($LAN4)."/ipv6/validlft") == "") $LanDhcpLifeTime = "";
		else $LanDhcpLifeTime = query(INET_getpathbyinf($LAN4)."/ipv6/validlft")/60;
	}	
}	
