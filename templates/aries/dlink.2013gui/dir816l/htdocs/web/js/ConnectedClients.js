...
function GetResult_RouterIp(result_xml)//kity  Router IP.
{
	var reserveIP =$("#client_IPAdrReserve").val();
	var reserveIPnums = reserveIP.split(".");
	if (result_xml != null)
	{
		var routerIP = result_xml.Get("GetRouterLanSettingsResponse/RouterIPAddress");
		var routerIPnums = routerIP.split(".");
		if (routerIPnums[3] == reserveIPnums[3])
		{
			return false;
		}
	}
...
function closeEditPOP(id)
{
	...
	HNAP.GetXMLAsync("GetRouterLanSettings", null, "GetValue", function(xml)//kity
	{
		if(GetResult_RouterIp(xml) == false)
		{
			document.getElementById("IPAdrReserve_Info").innerHTML= I18N("j","IP Address should not be the same as LAN IP Address.");
			return;
		}
		else
		{
