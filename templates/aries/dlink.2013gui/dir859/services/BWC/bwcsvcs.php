...
function bwc_tc_spq_2013gui_start($rtbwcp, $name, $ifname)
{
    ...
    if($prefix == "WAN") {
//There is a bug that psts -i always get 1000F, need fix!!
        $linkstatus = get("", PHYINF_getruntimephypath($name)."/linkstatus");
        if($linkstatus=="10F" || $linkstatus=="10H")           {$trate = 10000.$unit;  }//Kbps for unit.
        else if ($linkstatus=="100F" || $linkstatus=="100H")   {$trate = 100000.$unit; }
        else if ($linkstatus=="1000F" || $linkstatus=="1000H") {$trate = 1000000.$unit;}
        else $trate = 1000000.$unit;
    }
