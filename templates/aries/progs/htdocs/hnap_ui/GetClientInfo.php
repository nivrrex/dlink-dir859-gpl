			...
			while($line != "")
			{
				...
				if(strstr($subline, "no")!="") //It means not in local.
				{
				  ...
				  if($ipaddr!="" && $ipaddr!="0.0.0.0" && $ipaddr!=INF_getcurripaddr($LAN1) && $ipaddr!=INF_getcurripaddr($LAN2))
				  {
						...
						else if($br=="br0" && $portnumber==4)	{$type=$br0_p4_type;}
						...
						if(strstr($type, "2.4G")!="")		{$band="2.4G";}
						else if(strstr($type, "5G")!="")	{$band="5G";}
						else								{$band="";}
					...
					echo "		<IPv6Address>".getIPv6AddrByMAC($mac)."</IPv6Address>\n";	//+++ HuanYao Kang.
						...
						echo "		<Band>".$band."</Band>\n";
						echo "		<SignalLevel>".getSignalStrength($mac)."</SignalLevel>\n";
						echo "	</ClientInfo>\n";
					}
				}	
