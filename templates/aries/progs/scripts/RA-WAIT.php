...
function dhcp_client($mode, $inf, $devnam, $opt, $router, $dns)
{
	...
	} else if ($mode=="SITDHCP" && $previnf!="") {
		$sitdev = PHYINF_getruntimeifname($previnf);
		if ($sitdev=="") return error("no SIT device.");
		msg("SIT device = ".$sitdev);
	}
	msg("dhcpopt: ".$opt);
	...
	else if ($mode == "SITDHCP") $dname = $sitdev;
	else $dname = $devnam;
	...
	if ($pppdev != "")
		...
		cmd("dhcp6c -c ".$cfg." -p ".$pid." -t LL -o ".$devnam." -n ".$inf." ".$sitdev);
	else
...
function main_entry($inf, $phyinf, $devnam, $dhcpopt, $dns, $me)
{
	...
	if(strstr($prevdevnam,"ppp")=="" && strstr($prevdevnam,"sit")=="") msg($inf."/".$devnam.", M=[".$mflag."], O=[".$oflag."]");
	...
	if($child!="")
	{
		...
		if(strstr($prevdevnam,"ppp")=="")
		{
			if(strstr($prevdevnam,"sit")=="") {
				...
				dhcp_client("SITDHCP", $inf, $devnam, $dhcpopt, $router, $dns);
			}
		}
		else
		{
