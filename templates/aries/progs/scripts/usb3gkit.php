...
if ($SLOT != "USB-1")
{
	/* This board has only one USB which we can support USB 3G adapter. */
	echo "echo USB3GKIT: We DO NOT support slot - ".$SLOT." !!! > /dev/console\n";
	echo "exit 9\n";
}
else
{
	if ($ACTION=="addttydataport")
	{
		if ($DEVNUM=="")
		{
			echo "echo USB3GKIT: No DEVNUM !!! > /dev/console\n";
			echo "exit 9\n";
		}
		else
		{
...
//			setattr("operator",  "get", "usb3gkit -c operator -v 0x".$VID." -p 0x".$PID." -n ".$DEVNAME);
	...
	else if ($ACTION=="deltty")
	{
		...
		{
			TRACE_debug("usb3gkit.php: devpath=".query("devpath").", DEVPATH=".$DEVPATH);
