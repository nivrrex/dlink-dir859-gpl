...
if ($action=="add")
{
	...
	if ($diskp=="")
	{
		...
		{	$serial = "";}
		if (isfile("/sys/block/".$prefix."/../../../../../../idProduct")=="1")
		{	$idProduct = fread("", "/sys/block/".$prefix."/../../../../../../idProduct");}
		else
		{	$idProduct = "";}
		if (isfile("/sys/block/".$prefix."/../../../../../../idVendor")=="1")
		{	$idVendor = fread("", "/sys/block/".$prefix."/../../../../../../idVendor");}
		else
		{	$idVendor = "";}
		...
		set($diskp."/size",		$size);
		set($diskp."/idProduct",		strip($idProduct));
		set($diskp."/idVendor",		strip($idVendor));
	}
	if ($pid!="0"||$fs!="UNKNOWN")
	{
		/* add new entry */
