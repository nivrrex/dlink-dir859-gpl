...
function ifsetup($name)
{
	...
	{
		$path_run_inf_wan2 = XNODE_getpathbytarget("/runtime", "inf", "uid", $WAN2, 0); //WAN2 DHCP test for dual WAN
		$dhcp_ipaddr  = query($path_run_inf_wan2."/inet/ipv4/ipaddr"); 
	}
	if ($infp == "")
	{
	...
	*/
	...
	if($addrtype != "ipv4" && $addrtype != "ipv6")
	{
		...
		if($dialmode == "manual")
		{
			startcmd("event WAN-1.PPP.MANUAL");
		}
	}
	...
	if ($lower=="")
	{
		...
		else if	($addrtype == "ppp4")
		{
			if(query("/tr069/external/enable_dualwan")=="1")
			{
				if($dhcp_ipaddr !="") // if dhcp get ip. use combo ppp4 for dual WAN.
		...
		else if	($addrtype == "ppp6" || $addrtype == "ppp10") dophp("load", "/etc/services/INET/inet_ppp6.php");
	}
	else
	{
...
function ifinetsetup($name, $infp)
{
	...
	else
	{
		...
		fwrite(a, $_GLOBALS["START"], 'event WAN-1.PPP.SCHEDULE\n');
	}
