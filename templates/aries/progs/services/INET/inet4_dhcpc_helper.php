...
if ($ACTION=="bound")
{
	...
	if (query($sts."/udhcpc/inet")==$INET)
	{
		...
		if(isfile("/usr/sbin/tr069c") == 1){
			if(query("tr069_acs_url") != $TR069ACSURL || 
			   query("tr069_provisioning_code") != $TR069PROVISIONINGCODE){
			   $nochange = 0;
			}
		}
	}
	...
	if ($nochange!=1)
	{
		...
		/* TR069 info*/
		if(isfile("/usr/sbin/tr069c") == 1){
			set("tr069_acs_url",			$TR069ACSURL);
			set("tr069_provisioning_code",	$TR069PROVISIONINGCODE);
		}
