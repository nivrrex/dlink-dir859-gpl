...
#define LEAST_COMPRESS_SIZE	(512)
//==========================================================================
//
...
//+++ asize: bit0~28: actually file size.
//			 bit29~31 compressed mode.
//				0: no compress. 1: GZIP
//				2: LZMA			3~: Reserved.
//
typedef struct romfs_node {
    ...
    LONG	ctime;		// 18
    LONG	data_offset;	//22
    long	asize;		// 26 //++add by siyou, for actual file size.
    char	pad[4];		// 32
...
typedef struct node {
    ...
    size_t size;		//+++ compressed size  //Size of file/directory/link
    size_t asize;		//+++ add by siyou. Size of file/directory/link
...
void my_convert_fname(char *newname,const char *fname)
{
char *p;

	strcpy(newname, "./.mkromfs/");
...
//+++ siyou.
//this function return the compressed file size | with compress method.
//as asize.
ULONG get_zstat(char *fname)
{
	char newpath[1024];
	char cmd[1024];
	struct stat a,b;
	...
	my_convert_fname(newpath, fname);
	//printf("fname = %s\n", fname);
	//printf("newname = %s\n", newpath);

	//here, I'm lazy to do asize = ((COMPRESS_METHOD <<29) | file_size)
	//someone may can do this.
	sprintf(cmd, "lzma -%d -c -z %s | cat > %s", cratio, fname, newpath);
	system(cmd);
	get_status(fname,&a);
	get_status(newpath,&b);

	char *p = fname + strlen(fname);
	while(p > fname) { if (*p == '/') { p++; break;} else p--; }
	ratio = b.st_size * 100 / a.st_size;
	verb_printf(VERB_SUB, "compressed size=%ld  ratio=%d - %s\n",
				b.st_size, (int)ratio, p);
	//exit(1);
	return b.st_size;
}
...
static node * GetNodeInfo( const char *path, const char *name, int *hlink ) {
	...
	*last_multilink_p = node;
    ...
    //+++ modify by siyou. 2009/8/25 12:20pm
    //node->size = IS_DIRECTORY( stbuff.st_mode ) ? 0 : stbuff.st_size;
    if (IS_DIRECTORY( stbuff.st_mode ))
    {
    	//if asize = 0, mean content not compressed.
    	node->asize = node->size =0;
	}
	else
	{
		if (stbuff.st_size < LEAST_COMPRESS_SIZE)
    	{	node->size = stbuff.st_size;
    		node->asize = 0;
    	}
    	else
    	{
    		node->asize = stbuff.st_size;
    		node->size = get_zstat(newpath);
    	}
	}
	//-----
...
static void WriteNode( int fd, node *np ) {
    ...
    outputlong( (unsigned char*) &anode.asize, np->asize );

    sprintf( padhere, "%4d", np->nodenum );
    memcpy( anode.pad, padhere, 4 );
...
static void WriteNodeTable( int fd ) {
    ...
    strcpy( header.name, "ROMFS v9.0" ); //+++ siyou. v9.0 is my magic number.
...
static void WriteData( int fd, node *np ) {
    ...
    {
    char cname[1024];
    	my_convert_fname(cname, np->path);
    	//printf("write %s %d bytes\n", cname, np->asize);
    	if ( (ffd=open(cname, O_RDONLY | O_BINARY )) < 0 )
...
static void usage(void) {
    ...
    fprintf(stderr," -c         compression ratio from 1 to 9\n");
    fprintf(stderr,"\n");
    exit(EXIT_ARGS);
}
