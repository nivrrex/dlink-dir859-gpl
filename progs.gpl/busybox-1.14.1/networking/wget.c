...
//usage:     "\n	-T SEC	Network read timeout is SEC seconds (default timeout is 30 seconds)"
//usage:	)
...
//			bb_error_msg_and_die("bad redirection (no Location: header from server)");
	...
	}else if (G.content_len)		/* Gold for wget -s option */
...
#if ENABLE_FEATURE_WGET_TIMEOUT
	//G.timeout_seconds = 900; /* original version */
	G.timeout_seconds = 30;    /* alpha version */
