...
static int xconnect_port_ipv6(void)
{	char *buf_ptr;
	unsigned port_n;
	if (ftpcmd("EPSV", NULL) != 229)
	    ...
	    if (buf_ptr) *buf_ptr = '\0';
		   buf_ptr = strrchr(buf, '(');
	    	 buf_ptr = strchr(buf, '|');
		 port_n=atoi(buf_ptr+3);
		 return port_n;
}
#endif
static int xconnect_port_ipv4(void)
{	char *buf_ptr;
	unsigned port_n;
	if (ftpcmd("PASV", NULL) != 227) 
		{
		 ...
		 /* Response is "NNN garbageN1,N2,N3,N4,P1,P2[)garbage]
		    ...
		    buf_ptr = strrchr(buf, ',');
		    *buf_ptr = '\0';
		    port_n = xatoul_range(buf_ptr + 1, 0, 255);

		    buf_ptr = strrchr(buf, ',');
		    *buf_ptr = '\0';
		    port_n += xatoul_range(buf_ptr + 1, 0, 255) * 256;
			return port_n;
}
static int xconnect_ftpdata(void) {
    ...
    */
    #if ENABLE_FEATURE_IPV6
	  if(ipv6_open==1)
	  	port_num= xconnect_port_ipv6();
   	else
		port_num= xconnect_port_ipv4();
   #else
   	port_num= xconnect_port_ipv4();
   #endif  
...
#define FTPGETPUT_IPV6 0x800
#else
#define FTPGETPUT_IPV6 0x400
#endif
/*ftp_tftp_FW_CG_20100119 log_luo*//*End }*/
...
#if ELBOX_PROGS_GPL_TFTP_FTP_GET_PUT_FW_CONFIG /*ftp_tftp_FW_CG_20100119 log_luo*//*start {*/
#ifdef ELBOX_PROGS_PRIV_CLI_EXTENSION_FOR_BID_CLOUD_SEA
    opt = getopt32(argv, "cftdvu:p:P:mnl6", &user, &password, &port,
                   &verbose_flag, &do_continue,);
#else
    opt = getopt32(argv, "cftdvu:p:P:mn6", &user, &password, &port,
