...
static char **fill_envp(struct dhcpMessage *packet)
{
    ...
    else
    {
        ...
        if((temp = get_option(packet , DHCP_VENDOR_43)))
        {
            int total_len = *(temp-1);
            int	sub_header_len = 2, sub_data_len = 0, total_sub_len = 0;

            while(total_len > 0)
            {
                num_options++;
                sub_data_len = *(temp+OPT_LEN);
    ...
    if((temp = get_option(packet , DHCP_VENDOR_43)))
    {
        int total_len = *(temp-1), data_len = 0;
        int	sub_header_len = 2, sub_data_len = 0, total_sub_len = 0;
        ...
        while(total_len > 0)
        {
            ...
            if(*(temp+OPT_CODE) == 1)
            {
                data_len = strlen("tr069_acs_url=")+sub_data_len+1;
                envp[j] = xmalloc(data_len);
                snprintf(envp[j++],data_len,"tr069_acs_url=%s",(char*)(temp+OPT_DATA));
            }
            else if(*(temp+OPT_CODE) == 2)
            {
                data_len = strlen("tr069_provisioning_code=")+sub_data_len+1;
                envp[j] = xmalloc(data_len);
                snprintf(envp[j++],data_len,"tr069_provisioning_code=%s",(char*)(temp+OPT_DATA));
            }
            total_len -= total_sub_len;
            temp += total_sub_len;
        }
...
#if 1 // dump env to file if use -F ,add by traveller 2014-5-6
    if(client_config.envfile != NULL)
    {
        FILE *fp=fopen(client_config.envfile,"w");
        for ( i=0; i<j; i++ )
        {
            fprintf(fp,"%s\n",envp[i]);
        }
