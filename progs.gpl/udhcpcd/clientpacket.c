...
static int add_tr069_cpe_option(unsigned char *optionptr)
{
    unsigned char add_option[256];
    char *identifier ="dslforum.org";
    int len = strlen(identifier);

    *(add_option+OPT_CODE) = DHCP_VENDOR;
    *(add_option+OPT_LEN)  = len;
    memcpy((add_option+OPT_DATA),(unsigned char*)identifier,len);
...
#endif
#ifdef TR069_CPE_OPTION
    add_tr069_cpe_option(packet.options);
#endif
    add_requests(&packet);
