                  ...
                  u_int32_t dest_ip, int dest_port)
{
    ...
    if (bind(fd, (struct sockaddr *)&client, sizeof(struct sockaddr)) == -1)
    {
        ...
        //+++ michael_lee: (2013.04.29 17:37:20)
        // super dmz will cause can not bind wan ip. so reset source_ip. let routing decied it.
        memset(&client.sin_addr.s_addr, 0, sizeof(client.sin_addr.s_addr));
        if (bind(fd, (struct sockaddr *)&client, sizeof(struct sockaddr)) == -1)
        {
            LOG(LOG_DEBUG, "Sending renew bind failed...");
            return -1;
        }
#else
        return -1;
