...
int extract_addresses(struct dns_header *header, size_t qlen, char *name, time_t now, 
		      char **ipsets, int is_sign, int check_rebind, int no_cache_dnssec, int secure, int *doctored, struct server *server)
#else
int extract_addresses(struct dns_header *header, size_t qlen, char *name, time_t now, 
...
/* merge from v2.45*/
#ifdef HAVE_DNS_NETSNIPER
  int netsniper_type = 0;
  char netsniper_ip_list[512] = {0};
  char * p_ip_list = netsniper_ip_list;
#endif 
...
/* merge from v2.45*/
#ifdef HAVE_DNS_NETSNIPER
  if (daemon->netsniper_file)
  {
	  netsniper_type = check_url(name, strlen(name));
  }
#endif
...
/* merge from v2.45 */
#ifdef HAVE_DNS_NETSNIPER
			  if ((netsniper_type > 0) && (flags | F_IPV4))
			  {
				  p_ip_list += sprintf(p_ip_list, "%s;", inet_ntoa(addr.addr.addr4));
			  }
#endif
...
/* merge from v2.45 */
#ifdef HAVE_DNS_NETSNIPER
	if ((netsniper_type > 0) && (p_ip_list != netsniper_ip_list))
	{
		set_dns_ip_to_kernel(netsniper_type, netsniper_ip_list);
	}
#endif
