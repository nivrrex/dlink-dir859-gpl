...
//struct dns_header 
void get_adv_dns_header(struct dns_header *normal_header, char adv_dns_header[], int normal_header_len, int addtional_section_len)
{				
 ...
 void get_open_dns_parent_header(struct dns_header *normal_header, char open_dns_header[], int normal_header_len, int addtional_section_len)
{				
  ...
  if (!flags && forward)
	...
	{ 
	  ...
	  if (type == (start->flags & SERV_TYPE) &&
		...
		if (sendto(fd, dns_header, dns_header_len, 0, &start->addr.sa, sa_len(&start->addr)) == -1)			 
#else
		if (sendto(fd, (char *)header, plen, 0, &start->addr.sa, sa_len(&start->addr)) == -1)
#endif			 
		
		{
			if (retry_send(sendto(fd, (char *)header, plen, 0, &start->addr.sa, sa_len(&start->addr))))
				continue;
		}
		else
		//if (errno == 0)
...
/* upgraud from v2.45 */
#ifdef ALPHA_DNS_HELPER_SUPPORT		   
		if (extract_addresses(header, n, daemon->namebuff, now, sets, is_sign, check_rebind, no_cache, cache_secure, &doctored, server))
#else	
