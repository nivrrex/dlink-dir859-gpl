...
#ifdef CONFIG_ENCRYPT_FW_ENABLE	
	if ((!user)&&(!passwd)&&(!ifname))
	{
		/*Just exit if no parameters are assigned. Added by Builder 2014/04/10.*/
		exit(-1);
	}
#endif /*CONFIG_ENCRYPT_FW_ENABLE*/
...
//      {perror("daemon");exit(-1);}
    ...
    do
    {
        ...
        if(selret==0)
        {
            ...
            {
#ifdef CONFIG_ENCRYPT_FW_ENABLE
				fprintf(stderr,"MFCD: timeout terminated !!\n");
#else
