...
//we need add link layer address option (tom, 20130626)
...
buildsol (solicit_packet *rs, struct sockaddr_in6 *tgt, const char *ifname)
{
	...
	//we cannot get mac address, so skip link layer address option (tom, 20130626)
	if(getmacaddress(ifname, rs->hw_addr) != 0)
		return sizeof(struct nd_router_solicit);

	//we need link layer address option, add it (20130626)
