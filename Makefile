...
ROOTFS_DIRS:= etc etc/init.d etc/config etc/scripts etc/defnodes var
endif
...
ifeq ($(strip $(ELBOX_ECOS)),y)
	$(Q)echo "upgrade"					> $(TARGET)/etc/config/devconf
endif
...
gpl_license:
	@echo -e "\033[32mGenerating GPL license files ...\033[0m"
	$(Q)make -C progs.gpl license
	$(Q)make -C progs.brand license

gpl_license_clean:
	$(Q)make -C progs.gpl license_clean
	$(Q)make -C progs.brand license_clean
...
.PHONY: progs_prepare progs progs_install progs_clean gpl_license gpl_license_clean
...
env_clean:
	...
	$(Q)rm -rf kernel progs.template progs.board progs.brand  $(PATH_MK) $(CONF_MK) $(ARCH_MK)
ifneq ($(strip $(ELBOX_ECOS)),y)
	$(Q)rm -rf gendef.sh
endif
...
update:
	...
	$(Q)if [ "$(ELBOX_MINIDLNA)" == "y" ] && [ "$(ELBOX_PROGS_GPL_ITUNES_SERVER)" == "" ] ; then \
		echo -e "\033[33miTunes server is disabled but minidlna is enabled. Need to check itunes server for id3tag...\033[0m" ; \
		$(SVN_UP) progs.gpl/itunes_server/ ; \
	fi
...
BUILD_MYDLINK_SCRIPT = mydlink_sdk.sh
MYDLINK_SDK_DIR = mydlink_sdk
build_mydlink_sdk:
	$(Q)if [ ! -d $(MYDLINK_SDK_DIR) ];  then svn up $(MYDLINK_SDK_DIR);  fi
	$(Q)make mdconfig
	$(Q)cp mdconfig.mk $(MYDLINK_SDK_DIR)
	$(Q)make -C $(MYDLINK_SDK_DIR) tool
	$(Q)cp $(MYDLINK_SDK_DIR)/$(BUILD_MYDLINK_SCRIPT) ./
	$(Q)./$(BUILD_MYDLINK_SCRIPT) build

.PHONY: mydlink mdconfig build_mydlink_sdk
endif
...
ifdef ELBOX_BRAND_ARIES_DLINK_2013GUI
LOCALE_SQFS := jslang.sqfs
LOCALE_TARGET := $(TARGET)/htdocs/web/js/localization

ifdef FIRMWARENAME
LNG_MODELNAME := $(FIRMWARENAME)
else
LNG_MODELNAME := $(ELBOX_MODEL_NAME)
endif

LNG_VERSION= $(shell echo $(ELBOX_FIRMWARE_VERSION) | cut -d. --output-delimiter=\"\" -f1,2)
LNG__FILENAME:=$(shell echo $(LNG_MODELNAME)_$(LNG_VERSION)$(ELBOX_FIRMWARE_REVISION))
LANGUAGE_MTDBLOCK := $(shell cat $(TARGET)/etc/config/langpack)

langpack: $(MKSQFS) ./tools/seama/seama
ifndef MKSQFS
	@echo -e "\033[32mboard config.mk do not define MKSQFS,we do not know how to make squashfs...\033[0m"
	exit 9
endif

ifeq ($(strip $(LANGUAGE_MTDBLOCK)),)
	@echo -e "\033[32mwe do not know language pack which mtdblack to put.check /etc/config/langpack...\033[0m"
	exit 9
endif

	$(Q)if [ ! -d $(LOCALE_TARGET) ];	then echo -e "\033[31mDirectory $(LOCALE_TARGET) does not exist!please build rootfs first\033[0m"; exit 9; fi
	@echo -e "\033[32mStart building language file with version ...\033[0m"

	$(Q)echo $(ELBOX_FIRMWARE_VERSION)$(ELBOX_FIRMWARE_REVISION) > $(TARGET)/htdocs/web/js/localization/version
	$(Q)echo "$(shell date +"%F %T")" > $(TARGET)/htdocs/web/js/localization/builddate
	
	$(Q)rm -f $(LOCALE_SQFS)
	$(Q)rm -f $(LNG__FILENAME).lng
	
	$(Q)$(MKSQFS) $(LOCALE_TARGET) $(LOCALE_SQFS) > /dev/null
	$(Q)$(SEAMA) -i $(LOCALE_SQFS) -m dev=$(LANGUAGE_MTDBLOCK) -m type=language -m noheader=1 -m signature=$(ELBOX_CONFIG_SIGNATURE) 
	$(Q)$(SEAMA) -d $(LOCALE_SQFS).seama
	$(Q)mv $(LOCALE_SQFS).seama images/$(LNG__FILENAME).lng
	
	$(Q)ls -l images/$(LNG__FILENAME).lng
	@echo -e "\033[32myour system must be the seama header ...\033[0m"
